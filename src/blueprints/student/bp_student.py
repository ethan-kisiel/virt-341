...
def profile(student_id: int = None):
    ...
    return render_template(
        "student-profile.html", student=student, user=user, include_navbar=True
    )


@student_bp.route("/profile/<int:student_id>/save", methods=["PATCH"])
@login_required
def save_profile(student_id: int = None):
    """Saves teh user's profile
...
@student_bp.route("/profile/<int:student_id>/delete", methods=["DELETE"])
@login_required
def delete_profile(student_id: int = None):
